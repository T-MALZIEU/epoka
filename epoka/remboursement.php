...
$req = "SELECT missid,missdebut,missfin,missnom,misspayee,comnom,comCP,salnom,salprenom FROM mission,commune,salarie WHERE mission.missidcommune=commune.comid AND missidsalarie = salid AND salidsuperieur = :matrsup AND missvalide = 1";

$id =$_SESSION['superieur'];

$stmt = $pdo->prepare($req);
$stmt->bindParam(":matrsup",$id,PDO::PARAM_STR);
    $stmt->execute();
...
echo "<table class='missions' border='1'><tr><th>CHARGÉ DE MISSION</th><th>NOM</th><th>DEBUT</th><th>FIN</th><th>DESTINATION</th><th>COUT</th><th>REMBOURSÉE</th></tr>";
...
while($row = $stmt->fetch(PDO::FETCH_ASSOC))
{
    ...
    else{echo "<td > <a  href='serv_remboursement.php?idmission=".$row['missid'] ."'>REMBOURSER</a> </td>";}
